...
AltSoftSerial aSoftSerial;
#define DebugSerial Serial
#define XBeeSerial aSoftSerial
...
void loop() {
  if (XBeeSerial.available()) {
    DebugSerial.println("Something is available!");
    char data[80];
    uint8_t column;
    ...
    for (column = 0; column < sizeof(data); ++column) {
      ...
      //DebugSerial.print((char)b);
      //DebugSerial.write(' ');
      DebugSerial.write((char)b);
    }
  ...
  if ( DebugSerial.available() ) {
    ...
    while (DebugSerial.available()) {
      char cByte;
      cByte = DebugSerial.read();
      sStr += cByte;
    }    
  }

  if ( sStr.length() > 0 ) {
    DebugSerial.print("Sending string to XBee:");
    DebugSerial.println(sStr.c_str());
    XBeeSerial.println(sStr);
    //XBeeSerial.write('a');
  }
}
