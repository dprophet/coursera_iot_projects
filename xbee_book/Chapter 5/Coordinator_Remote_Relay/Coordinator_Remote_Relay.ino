...
//
// This example shows how to control the heating through a relay
// connected to a remote standalone XBee module (no Arduino attached),
// using remote AT commands.
//
...
// Switch the heating by toggling pin 1 on a remote, standalone XBee
// module (connected to a relay, for example).
void switchHeating(bool state) {
  if (setRemotePin(0x0013A20040F51631, 1, state)) {
    heating_state = state;
    publish(F("House/Heating"), state);
  }
}

// Set remote pin 1 to the given value
bool setRemotePin(XBeeAddress64 addr, uint8_t pin, bool state) {
  uint8_t command[2];
  // Pin <10 are controlled using Dx commands, pins 10+ using Px commands
  if (pin < 10) {
    command[0] = 'D';
    command[1] = '0' + pin;
  } else {
  ...
  uint8_t value = state ? 5 : 4;

  RemoteAtCommandRequest request(addr, command, &value, sizeof(value));
  uint8_t status = xbee.sendAndWait(request, 5000);
  return (status == 0);
}
