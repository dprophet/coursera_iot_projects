...
//
// This example shows how the coordinator Arduino can use a remote standalone
// XBee module (no Arduino attached) to monitor a window reed switch, to
// detect when a window is opened, and disable the heating if so.
//
// This sketch does not actually control the heating, but instead
// just shows the intended state on the serial console.
//
// Make sure to modify the code below to match your list of rooms and
// corresponding XBee addresses for the sensor nodes.
//
...
// Last received window states
bool open_windows[NUM_ROOMS] = {false, false};
// Current setpoint
float setpoint = NAN;
// Heating state
...
// Only print and publish the intended state, do not control anything yet
void switchHeating(bool state) {
  Serial.print(F("Heating should be "));
  Serial.println(state ? F("on") : F("off"));
  heating_state = state;
  publish(F("House/Heating"), state);
}

void processIoSample(ZBRxIoSampleResponse& rx, uintptr_t) {
  XBeeAddress64 addr = rx.getRemoteAddress64();
  if (addr == 0x0013A20040B1924F) {
      bool open = rx.isDigitalOn(1);
      open_windows[LIVINGROOM] = open;
      inputs_changed = true;
      publish(F("Livingroom/Window"), open);
      return;
  }
  if (addr == 0x0013A20040B19240) {
      bool open = rx.isDigitalOn(1);
      open_windows[STUDY] = open;
      inputs_changed = true;
      publish(F("Study/Window"), open);
      return;
  }

  DebugSerial.println(F("Unknown IO sample"));
...
void setup() {
  ...
  xbee.onZBRxIoSampleResponse(processIoSample);
